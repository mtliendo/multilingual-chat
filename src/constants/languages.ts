...
export const detectLanguage = (text: string): string => {
  ...
  if (
    /\b(the|is|and|to|of|a|in|that|have|i|it|for|not|on|with)\b/.test(lowerText)
  ) {
    return "en"
  }
  if (
    /\b(el|la|que|de|y|a|en|un|ser|se|no|haber|por|con|su|para)\b/.test(
      lowerText
    )
  ) {
    return "es"
  ...
  if (
    /\b(der|die|das|und|zu|sie|ich|in|den|mit|ist|für|auf|nicht)\b/.test(
      lowerText
    )
  ) {
    return "de"
  ...
  if (
    /\b(le|la|les|de|et|je|tu|il|un|être|avoir|que|dans|ce|qui)\b/.test(
      lowerText
    )
  ) {
    return "fr"
